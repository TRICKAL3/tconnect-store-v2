...
/** One shared promise so startup + middleware don’t duplicate work. */
let shared: Promise<void> | undefined;

/**
 * Postgres only. Creates `public.user_cart_snapshots` on whatever DB Prisma's
 * DATABASE_URL points at (fixes “table does not exist” when SQL was run elsewhere).
 */
export function ensureUserCartSnapshotTable(prisma: PrismaClient): Promise<void> {
  const url = process.env.DATABASE_URL || '';

  if (url.startsWith('file:')) {
    console.warn('[cart] user_cart_snapshots: skipped (SQLite). Use Postgres or prisma migrate locally.');
...
`);
    ...
    console.warn(
      '[cart] user_cart_snapshots: FK to "User" failed (ok if constraint already matched). Confirm table "User" exists.'
    );
  }
