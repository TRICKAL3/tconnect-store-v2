...
/**
 * PawaPay Payment Page requires a public HTTPS return URL (not http://localhost).
 * Set PAWAPAY_RETURN_URL=https://your-store.com/checkout in backend/.env
 */
export function getPawapayReturnUrl(): { url: string | null; error?: string } {
  ...
  if (explicit) {
    if (!/^https:\/\//i.test(explicit)) {
      return {
        url: null,
        error:
          'PAWAPAY_RETURN_URL must be HTTPS (e.g. https://tconnect-store-v2.vercel.app/checkout).',
      };
    }
  ...
  if (!/^https:\/\//i.test(front) || /localhost|127\.0\.0\.1/i.test(front)) {
    return {
      url: null,
      error:
        'PawaPay requires an HTTPS return URL. Add to backend/.env: PAWAPAY_RETURN_URL=https://YOUR-LIVE-SITE/checkout (your Vercel URL or ngrok https URL for port 3180).',
    };
  }
...
export function userMessageForPawapayError(message: string, httpStatus?: number): string {
  ...
  if (
    ...
    return (
      'PawaPay rejected the API token. In backend/.env set PAWAPAY_API_TOKEN to your live token from ' +
      'https://dashboard.pawapay.io (API tokens), use PAWAPAY_MODE=live for production, then restart the API. ' +
      'Sandbox tokens only work with PAWAPAY_MODE=sandbox.'
    );
  }
