...
router.post('/', async (req: any, res) => {
  try {
    ...
    // For points payment: validate eligibility and balance (deduct on approval)
      ...
      try {
        ...
        if (order.userId) {
          await createUserNotification({
            ...
            message: `We received your order #${order.id.substring(0, 8)} ($${order.totalUsd.toFixed(2)}). We will review it shortly.`,
            link: '/orders',
          });
        }
        ...
        const payload =
          status === 'approved'
            ? {
                ...
                message: `Your order #${shortId} has been approved and is being processed.`,
              }
            : status === 'rejected'
              ? {
                  type: 'order_rejected',
              ...
              : {
                  ...
                  message: `Your order #${shortId} is complete — open Order History for codes and details.`,
                };
        await createUserNotification({
