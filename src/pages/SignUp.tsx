...
const SignUp: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      console.log('User already signed in, redirecting from SignUp page');
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        console.log('Firebase user detected on SignUp page, redirecting');
        setTimeout(() => {
          navigate('/', { replace: true });
        }, 500);
      }
    ...
    <div className="min-h-screen bg-dark-bg flex items-center justify-center px-4">
      <div className="card-dark p-6 sm:p-8 w-full max-w-md">
        ...
        <div className="mt-4 text-center text-sm">
          <p className="text-gray-400">Already have an account? <Link to="/signin" className="text-neon-blue">Sign in</Link></p>
        </div>
      </div>
    </div>
  );
